...
| `ccask -c "<chat>" "<question>"` | **Targeted:** ask one specific chat (repeat `-c`, or put chat names after the question). Reads that chat's **full transcript** extract — best fidelity for one chat. `-s` = cheaper cached summaries. |
