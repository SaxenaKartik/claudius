...
4. **`ccask "<question>"`** — the payoff: ask anything and Claudius searches *all* your past chats by default, answers, and cites which ones. Scope to specific chats with `ccask -c "Chat A" -c "Chat B" "<q>"` (or trailing names). Inside a chat, `/ccask <question>` does the same and lands the context in your session.
...
| `ccask "<question>"` | **Default: searches across *all* your chats** — ranks by relevance (+ recency/importance tie-breakers), answers from the top few, and **cites which chats** it used. **Query expansion is on by default**: Claude first suggests synonyms/abbreviations (*dead-letter queue* → also `dlq`, `redrive`) so it finds chats that used different words — semantic recall, no embeddings (`-E` or `$CCASK_EXPAND=0` to skip). Replies `CANNOT ANSWER:` if your chats don't cover it. Answer is styled on a TTY with a copy-to-clipboard prompt. |
| `ccask -c ["<chat>"] "<question>"` | **Targeted:** ask specific chat(s). Bare **`ccask -c`** opens a multi-select menu (Space ticks several, Enter confirms) then prompts for your question; or name them directly (repeat `-c`, or put chat names after the question). Reads the **full transcript** extract — best fidelity. `-s` = cheaper cached summaries. |
...
- **Reuse context (`ccfetch`)** — summarises a past chat into a cached handoff (goal / decisions / state / next steps / references), then either prints it, seeds a **new** session with it, or — via `/ccfetch` inside a chat — drops it straight into the session you're in. Summaries are cached and reused until the transcript changes.
- **Fork a chat (`ccbranch`)** — starts a **new** session seeded with a chat's *full* history, so you can branch off past work without touching the original.
- **Cross-chat ask (`ccask`, the default)** — retrieval is done **locally**: rank every session by relevance — over **conversation turns only** (injected memory / system-reminders are excluded so they can't skew relevance) — with keyword hits IDF-weighted so rare words like a project name outweigh common ones, plus an **IDF-weighted** boost for chats whose *first message* opens on the topic (a common opening word like "work" barely counts; a handoff-summary paste earns nothing), give a **topicality** boost to chats that mention a term *many times* (a chat that discussed DLQ throughout is about it, even if its first message wasn't — fixes mid-chat topic drift), then break near-ties by **recency** (older chats decay smoothly) and **importance** (chats you named/mapped rank higher) — a bounded, multiplicative bonus, so a clearly more-relevant chat always still wins. Then pull focused excerpts from the top few (round-robin so no single chat hogs the budget) and make **one** `claude -p` call to answer with citations. Tunable via `$CCASK_W_RECENCY`, `$CCASK_W_IMPORTANCE`, `$CCASK_RECENCY_DAYS`.
- **Semantic recall without embeddings (on by default; `-E` to skip)** — instead of a vector database, Claudius borrows Claude's semantic knowledge *once*: it expands your query into synonyms/abbreviations (*dead-letter queue* → `dlq`, `redrive`, `sqs`, `reprocess`) and feeds those into the same free local lexical ranker. You get meaning-based recall — finding a chat that used different words than your question — with no embedding model, vector store, or extra dependency. (True embeddings/RAPTOR remain a future option for larger histories.)
- **Compact-but-lossless extract** — every command that reads a chat (`ccask`, `ccfetch`, `ccspec`, `ccexplain`, `ccexport`) uses a cached text extract (~100× smaller than the raw JSONL, which is mostly tool/metadata noise), never the whole file — so they're all fast and cheap. The extract keeps the parts that matter for answering: message text, the **inputs** of each tool call (the command/query/path/URL you actually ran), the head **and** tail of long tool results, and a short slice of reasoning — so `ccask` can tell you *what* command fixed something, not just *that* a tool ran. (The extract needs `python3`; without it, commands fall back to handing the model the raw transcript — heavier, but still works.)
...
- **Config** — `CLAUDE_CONFIG_DIR` (default `~/.claude`), `CC_MAP` (default `$CLAUDE_CONFIG_DIR/cc_map.md`), `CCASK_TOPK` (how many chats `ccask` reads, default 5), `CCASK_EXPAND` (0 = no synonym expansion). Ranking weights are tunable too: `CCASK_W_RECENCY`, `CCASK_W_IMPORTANCE`, `CCASK_W_FREQ`, `CCASK_RECENCY_DAYS`, `CCASK_FMSG_ORIG`, `CCASK_FMSG_SYN`.
